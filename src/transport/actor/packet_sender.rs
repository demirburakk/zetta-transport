...
use crate::transport::state::{ConnectionState, UnackedPacket, UnackedPayload};
...
impl ZtConnectionActor {
    ...
    pub(super) fn flush_pacing_queue(&mut self) -> Option<std::time::Duration> {
        ...
        while let Some(payload) = self.state.unpaced_queue.front() {
            ...
            if self.state.pacing_tokens >= len {
                ...
                if let Err(e) = self.send_payload(p, 0) {
    ...
    pub(super) fn handle_retransmits(&mut self) -> Result<()> {
        let now = StdInstant::now();
        let base_rto = (self.state.rtt + self.state.rttvar * 4).max(Duration::from_millis(50));
        let mut to_retransmit = std::collections::HashSet::new();
        let mut to_drop = Vec::new();
        
        for (pn, up) in self.state.unacked_packets.iter_mut() {
            // Cap the multiplier at 8x (1 << 3) to avoid massive RTOs.
            let effective_rto = base_rto * (1 << up.retries.min(3));
            if now.duration_since(up.sent_at) > effective_rto {
                if up.is_mtu_probe {
                    to_drop.push(pn);
                } else {
                    up.retries += 1;
        ...
        for pn in to_drop {
            if let Some(up) = self.state.unacked_packets.remove(pn) {
                ...
                if up.is_mtu_probe {
                    ...
                    payloads_to_resend.push((up.payload, up.retries));
                }
            }
        }
    ...
    pub(crate) fn retransmit_payload(&mut self, payload: UnackedPayload, retries: u32) -> Result<()> {
        self.send_payload(payload, retries)
    }

    fn send_payload(&mut self, payload: UnackedPayload, retries: u32) -> Result<()> {
        let pn = self.state.get_next_packet_number()?;
        let kp = self.current_key_phase();
        let lowest_unacked = self.state.unacked_packets.keys().next().unwrap_or(pn);
        let (_, pn_len) =
        ...
        let is_initial = matches!(payload, UnackedPayload::Initial { .. });
        let is_handshake = matches!(payload, UnackedPayload::Handshake { .. });

        if is_initial {
            header.p_type = PacketType::Initial;
            header.is_long = true;
            header.version = 1;
            header.scid = self.state.scid.clone();
            header.key_phase = false;
        } else if is_handshake {
            header.p_type = PacketType::Handshake;
            header.is_long = true;
            header.version = 1;
            header.scid = self.state.scid.clone();
            header.key_phase = false;
        ...
        match &payload {
            ...
            UnackedPayload::Initial { cookie } => {
                ...
                .encode(&mut packet);
                if let Some(c) = cookie {
                    Frame::Cookie { cookie: c.clone() }.encode(&mut packet);
                }
                let pad_len = 1200usize.saturating_sub(packet.len() + 16);
                if pad_len > 0 {
                    Frame::Padding(pad_len).encode(&mut packet);
                }
            }
            UnackedPayload::Handshake { public_key, ed_public_key, transcript_hash, signature } => {
                Frame::Handshake {
        ...
        if is_initial || is_handshake {
            let is_client = is_initial; // client sends initial, server sends handshake
            let crypto = crate::crypto::CryptoContext::initial(&header.dcid, is_client);
            {
                let p = packet.as_mut();
