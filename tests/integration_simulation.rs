...
async fn test_network_simulations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    run_transfer_test("LossTest_10pct", SimulationConfig::new(10, 0, 0)).await?;
    println!("=== SCENARIO 1 PASSED in {:?} ===\n", start.elapsed());

    // --- SCENARIO 2: 20% REORDERING (50ms delay) ---
    println!("\n=== RUNNING SCENARIO 2: 20% REORDERING (50ms delay) ===");
    let start = std::time::Instant::now();
    run_transfer_test("ReorderTest_20pct_50ms", SimulationConfig::new(0, 20, 50)).await?;
    ...
    let start = std::time::Instant::now();
    run_transfer_test(
        "JointTest_5pctLoss_15pctReorder",
        SimulationConfig::new(5, 15, 30),
    )
    .await?;
